...
async def get_analytics_overview(
    ...
    # Current period
    total_resumes = db.query(models.Resume).filter(models.Resume.uploaded_at >= date_limit).count()
    total_jobs = db.query(models.Job).filter(models.Job.posted_at >= date_limit).count()
    total_users = db.query(models.User).filter(models.User.created_at >= date_limit).count()
    total_applications = db.query(models.Application).filter(models.Application.applied_at >= date_limit).count()
    active_users = db.query(models.User).filter(models.User.last_active >= date_limit).count()
    avg_score = db.query(func.avg(models.Application.match_score)).filter(models.Application.applied_at >= date_limit).scalar() or 0.0

    # Previous period for change calculation
    p_resumes = db.query(models.Resume).filter(models.Resume.uploaded_at >= prev_date_limit, models.Resume.uploaded_at < date_limit).count()
    p_jobs = db.query(models.Job).filter(models.Job.posted_at >= prev_date_limit, models.Job.posted_at < date_limit).count()
    p_apps = db.query(models.Application).filter(models.Application.applied_at >= prev_date_limit, models.Application.applied_at < date_limit).count()
    p_users = db.query(models.User).filter(models.User.created_at >= prev_date_limit, models.User.created_at < date_limit).count()
...
async def get_match_stats(
    ...
    apps = db.query(models.Application.match_score).filter(models.Application.applied_at >= date_limit).all()
    ...
    for app in apps:
        ...
        if score >= 80: dist["A'lo (80-100%)"] += 1
        elif score >= 60: dist["Yaxshi (60-79%)"] += 1
        elif score >= 40: dist["O'rtacha (40-59%)"] += 1
        else: dist["Past (0-39%)"] += 1
...
async def get_top_skills(
    ...
    date_limit = datetime.utcnow() - timedelta(days=days)
    all_jobs = db.query(models.Job.required_skills).filter(models.Job.posted_at >= date_limit).all()
    skill_counts = {}
    
    for job in all_jobs:
        if job.required_skills:
            # skills formatini tekshirish (JSON bo'lishi mumkin)
            import json
            try:
                skills = json.loads(job.required_skills)
                if isinstance(skills, str): skills = [s.strip().lower() for s in skills.split(",") if s.strip()]
            except:
...
async def get_monthly_stats(
    ...
    for i in range(5, -1, -1):
        ...
        resumes = db.query(models.Resume).filter(models.Resume.uploaded_at >= month_date, models.Resume.uploaded_at < next_month).count()
        jobs = db.query(models.Job).filter(models.Job.posted_at >= month_date, models.Job.posted_at < next_month).count()
        apps = db.query(models.Application).filter(models.Application.applied_at >= month_date, models.Application.applied_at < next_month).count()
...
async def export_applicants_to_excel(
    ...
    applicants = db.query(models.Application).filter(models.Application.job_id == job_id).all()
...
async def export_resumes(
    ...
    output.seek(0)
    return StreamingResponse(output, headers={'Content-Disposition': 'attachment; filename="all_resumes.xlsx"'}, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
async def export_jobs(
    ...
    output.seek(0)
    return StreamingResponse(output, headers={'Content-Disposition': 'attachment; filename="all_jobs.xlsx"'}, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
async def export_all_data(
    ...
    output.seek(0)
    return StreamingResponse(output, headers={'Content-Disposition': 'attachment; filename="full_report.xlsx"'}, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
