  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] p-6 lg:p-8 relative overflow-hidden font-['Outfit']">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-10">
        ...
        {/* Search & Filter */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 rounded-[2.5rem] bg-[var(--bg-surface)] border border-[var(--border-main)] shadow-2xl backdrop-blur-3xl">
          <div className="lg:col-span-8 relative group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-indigo-500 transition-colors" />
            <input
              type="text"
              placeholder="Kompaniya nomi yoki soha bo'yicha qidirish..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-[var(--bg-main)]/50 border border-[var(--border-main)] rounded-2xl py-4 pl-14 pr-6 text-[var(--text-main)] placeholder-slate-500 focus:ring-2 focus:ring-indigo-500/20 transition-all outline-none font-bold text-sm"
            />
          </div>
          <div className="lg:col-span-4">
             <select 
               ...
               className="w-full bg-[var(--bg-main)]/50 border border-[var(--border-main)] rounded-2xl py-4 px-6 text-[var(--text-main)] font-bold text-sm appearance-none outline-none focus:ring-2 focus:ring-indigo-500/20"
             >
               {industries.map(i => (
          ...
          <div className="space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {currentCompanies.map((company, idx) => (
                <div 
                  key={company.id}
                  className="group relative p-8 rounded-[2.5rem] bg-[var(--bg-surface)] border border-[var(--border-main)] shadow-xl hover:translate-y-[-8px] transition-all duration-500 flex flex-col items-center text-center overflow-hidden animate-in fade-in zoom-in-95"
                ...
                >
                  ...
                  <div className="relative mb-6">
                    <div className="w-24 h-24 rounded-3xl bg-[var(--bg-main)]/50 border border-[var(--border-main)] p-1 overflow-hidden transition-transform duration-500 group-hover:scale-110">
                      {company.logo_url ? (
                        <img src={company.logo_url} alt={company.company_name} className="w-full h-full object-cover rounded-[1.25rem]" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-indigo-500 bg-indigo-500/10 rounded-[1.25rem]">
                          <Building2 size={40} />
                        </div>
                      )}
                    </div>
                    {company.is_verified && (
                       <div className="absolute -right-2 -bottom-2 w-8 h-8 bg-indigo-600 rounded-full border-4 border-[var(--bg-surface)] flex items-center justify-center text-white shadow-lg">
                          <Verified size={14} />
                       </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 mb-6 w-full">
                    <h3 className="text-xl font-black text-[var(--text-main)] truncate uppercase tracking-tight">
                      {company.company_name}
                    </h3>
                    <div className="flex items-center justify-center gap-2 text-[var(--text-muted)] font-black text-[10px] uppercase tracking-widest text-indigo-400">
                      <TrendingUp size={12} />
                      {company.industry || 'Boshqa'}
                    </div>
                    <div className="flex items-center justify-center gap-2 text-[var(--text-muted)] font-bold text-xs">
                      <MapPin size={12} />
                      {company.location || 'Noma\'lum'}
                    </div>
                  </div>

                  <p className="text-[11px] text-[var(--text-muted)] leading-relaxed mb-8 line-clamp-2">
                  ...
                  <div className="mt-auto w-full space-y-3">
                    ...
                    <a 
                      ...
                      className="w-full py-3 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all inline-block"
                    >
                      Profilni ko'rish
                    </a>
                  </div>
